...
async def create_message(
    ...
    try:
        ...
        if request.model.startswith("provider-3/") or request.model.startswith("a4f/"):
            # For A4F, we need to keep the provider-3/ prefix but use openai/ for LiteLLM routing
            if request.model.startswith("a4f/"):
                # Convert a4f/ to provider-3/ format
                actual_model = request.model.replace("a4f/", "provider-3/")
            else:
                # Already has provider-3/ prefix
